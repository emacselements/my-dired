...
;; (defun dired-rename-files-python ()
;;   "Run the Python rename script on marked files/folders in Dired."
;;   (interactive)
...
;;     (if files
;;         (progn
;;           ;; Apply call-process with each file as a separate argument
;;           (apply 'call-process
;;                  "~/bin/rename_files.py"
;;                  nil     ; input
;;                  nil     ; output
;;                  nil     ; display
;;                  files)  ; arguments as a list

;;           ;; Unmark all files
;;           (dired-unmark-all-marks)

;;           ;; Save excursion and window configuration while reverting
;;           (let ((dired-auto-revert-buffer nil))
;;             (revert-buffer t t))

;;           ;; Restore exact position and window start
;;           (goto-char old-point)
;;           (set-window-start (selected-window) window-start))
;;       (message "No files marked. Mark files with 'm' or use '* t' to mark all."))))

;; ;; Bind it to "N" in Dired mode
;; (add-hook 'dired-mode-hook
;;           (lambda ()
;;             (define-key dired-mode-map (kbd "N") 'dired-rename-files-python)))
